...
def main():
    ...
    try:
        ...
        if operation == "check-unblocked":
            ...
            # Print summary
            blocked = [
                email for email, status in results.items() if "BLOCKED" in status
            ]
            unresolvable = [
                email for email, status in results.items() if "UNRESOLVABLE" in status
            ]
            allowed = [
                email for email, status in results.items() if "ALLOWED" in status
            ]
            ignored = [
                email for email, status in results.items() if "IGNORED" in status
            ]
            invalid = [
                email for email, status in results.items() if "INVALID" in status
            ]
            ...
            try:
                export_users_last_login_to_csv(
                    emails, token, base_url, output_file, batch_size, connection_filter
                )
            ...
            if env == "prod" and not confirm_production_operation(
            ...
            multiple_users: dict[
                str, list[str]
            ] = {}  # Store emails with multiple users
