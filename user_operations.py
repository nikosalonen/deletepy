...
def handle_rate_limit_response(response: requests.Response, attempt: int) -> bool:
    ...
    if response.status_code == 429:  # Too Many Requests
        if attempt >= MAX_RETRIES:
            print(
                f"{RED}Rate limit exceeded after {MAX_RETRIES} attempts. Stopping.{RESET}"
            )
            return False
        ...
        print(
            f"{YELLOW}Rate limit hit. Waiting {delay} seconds before retry {attempt}/{MAX_RETRIES}...{RESET}"
        )
        time.sleep(delay)
        return True
...
def make_rate_limited_request(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            delay = min(BASE_RETRY_DELAY * (2 ** (attempt - 1)), MAX_RETRY_DELAY)
            print(
                f"{YELLOW}Request failed, retrying in {delay} seconds... ({attempt}/{MAX_RETRIES}){RESET}"
            )
            time.sleep(delay)
...
def _validate_and_setup_export(
    ...
    if not validate_rate_limit_config():
        print(
            f"{YELLOW}Warning: Rate limit configuration may be too aggressive.{RESET}"
        )
            ...
            if user_details:
                csv_rows.append(
                    _build_csv_data_dict(
                        ...
                        user_id,
                        user_details,
                        f"MULTIPLE_USERS ({len(user_ids)})",
                    )
                )
            else:
                csv_rows.append(
                    _build_csv_data_dict(email, user_id, None, "ERROR_FETCHING_DETAILS")
                )
        return csv_rows, counters
...
def _process_email_batch(
    ...
    for idx, email in enumerate(batch_emails, batch_start + 1):
        ...
        email_csv_data, email_counters = _fetch_user_data(
...
def _write_csv_batch(csv_data: list[dict], output_file: str, batch_number: int) -> bool:
    ...
    try:
        ...
        print(
            f"{GREEN}✓ Batch {batch_number} saved to {CYAN}{output_file}{GREEN}{RESET}"
        )
        return True
...
def _build_csv_data_dict(
    email: str, user_id: str, user_details: dict | None, status: str
) -> dict:
...
def unlink_user_identity(
    ...
    if response is None:
        print(
            f"{RED}Error unlinking identity {CYAN}{user_identity_id}{RED} from user {CYAN}{user_id}{RED}: Request failed after retries{RESET}"
        )
        return False
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            print(
                f"{GREEN}Successfully unlinked identity {CYAN}{user_identity_id}{GREEN} ({provider}) from user {CYAN}{user_id}{GREEN}{RESET}"
            )
            return True
        else:
            print(
                f"{RED}Failed to unlink identity {CYAN}{user_identity_id}{RED} from user {CYAN}{user_id}{RED}: HTTP {response.status_code}{RESET}"
            )
            return False
    except Exception as e:
        print(
            f"{RED}Error unlinking identity {CYAN}{user_identity_id}{RED} from user {CYAN}{user_id}{RED}: {e}{RESET}"
        )
        return False


def _search_users_by_social_id(social_id: str, token: str, base_url: str) -> list[dict]:
    """Search for users with a specific social ID in their identities.

    Args:
        social_id: Social media ID to search for
        token: Auth0 access token
        base_url: Auth0 API base URL

    Returns:
        List of user info dictionaries for users found with this social ID
    """
    found_users = []
    ...
    params = {"q": f'identities.user_id:"{social_id}"', "search_engine": "v3"}

    response = make_rate_limited_request("GET", url, headers, params=params)
    if response is None:
        print(
            f"{RED}Error searching for social ID {CYAN}{social_id}{RED}: Request failed after retries{RESET}"
        )
        return found_users
    ...
    try:
        ...
        if users and isinstance(users, list) and len(users) > 0:
            for user in users:
                ...
                main_connection = (
                    main_identity.get("connection", "unknown")
                    if main_identity
                    else "unknown"
                )
        ...
        print(
            f"{RED}Error parsing search results for social ID {CYAN}{social_id}{RED}: {e}{RESET}"
        )
...
def _categorize_users(
    found_users: list[dict], auto_delete: bool = True
) -> tuple[list[dict], list[dict], list[dict]]:
    """Categorize users based on their identity configuration.

    Args:
        found_users: List of user info dictionaries
        auto_delete: Whether auto-delete logic should be applied

    Returns:
        Tuple of (users_to_delete, identities_to_unlink, auth0_main_protected)
    """
    users_to_delete = []
    ...
    if auto_delete:
        for user_info in found_users:
            main_connection = user_info["main_connection"]
            identities_count = user_info["identities_count"]

            if main_connection == "auth0":
                # Main identity is auth0 - protect entire user
                auth0_main_protected.append(user_info)
            elif identities_count == 1:
...
def _display_search_results(
    ...
    users_to_delete: list[dict],
    identities_to_unlink: list[dict],
    auth0_main_protected: list[dict],
    ...
    Args:
        ...
        found_users: All found users
        not_found_ids: List of social IDs that were not found
        users_to_delete: Users categorized for deletion
        identities_to_unlink: Users categorized for identity unlinking
        auth0_main_protected: Users with auth0 main identity (protected)
        auto_delete: Whether auto-delete logic is enabled
    """
    ...
    if identities_to_unlink and auto_delete:
        ...
        for user in identities_to_unlink:
            ...
            print(f"  Email: {CYAN}{user['email']}{RESET}")
            print(
                f"  Identity to unlink: {CYAN}{user['matching_connection']}{RESET}{main_status}"
            )
    ...
    other_users = [
        ...
        and u not in identities_to_unlink
        and u not in auth0_main_protected
    ]
    if other_users:
...
def _handle_auto_delete_operations(
    ...
    """Handle the auto-delete operations with confirmation and execution.

    Args:
        users_to_delete: Users to delete completely
        identities_to_unlink: Users with identities to unlink
        token: Auth0 access token
        base_url: Auth0 API base URL
        env: Environment (dev/prod) for confirmation prompts
        auto_delete: Whether auto-delete operations should be performed
    """
